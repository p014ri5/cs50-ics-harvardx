...
void substitute(string key)
{
    ...
    for (int i = 0; i < strlen(p); i++)
    {
        if (isalpha(p[i])) // checking p[i] is alphabet
        {
            ...
            if (islower(p[i]))
            {
                subchar(tolower(x), key); // passing p[i] in lower-case
            }
            else
            {
                subchar(toupper(x), key); // passing p[i] in UPPER-CASE
            }
...
void subchar(char pos, string key) // passing p[i] and upper/lower(alpha array)
{
